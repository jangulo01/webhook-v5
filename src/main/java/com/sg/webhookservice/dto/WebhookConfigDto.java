...
/**
 * DTO que representa una configuración de webhook,
 * incluyendo destino, estrategias de reintento y parámetros de seguridad.
 */
@Data
@Builder
@NoArgsConstructor
...
@Schema(description = "Configuración para un webhook")
public class WebhookConfigDto {
    ...
    @Schema(description = "Estadísticas de salud del webhook", accessMode = Schema.AccessMode.READ_ONLY)
    @JsonProperty(access = JsonProperty.Access.READ_ONLY)
    private WebhookHealthStatsDto healthStats;

    /**
     * Clase interna para establecer valores predeterminados de configuración
     */
    ...
    /**
     * Valida que la estrategia de backoff sea válida
     */
    @AssertTrue(message = "La estrategia de backoff debe ser 'linear', 'exponential' o 'fixed'")
    @JsonProperty(access = JsonProperty.Access.WRITE_ONLY)
    public boolean isValidBackoffStrategy() {
        if (backoffStrategy == null) return true;
        return backoffStrategy.equals("linear") || 
               backoffStrategy.equals("exponential") || 
               backoffStrategy.equals("fixed");
    }

    /**
     * Validación de consistencia para factor de backoff
     */
    @AssertTrue(message = "El factor de backoff es obligatorio para la estrategia exponencial")
    @JsonProperty(access = JsonProperty.Access.WRITE_ONLY)
    public boolean isValidFactorForExponential() {
        if (!"exponential".equals(backoffStrategy)) return true;
        return backoffFactor != null && backoffFactor > 1.0;
    }

    /**
     * Validación de consistencia para intervalos
     */
    @AssertTrue(message = "El intervalo máximo debe ser mayor que el intervalo inicial")
    @JsonProperty(access = JsonProperty.Access.WRITE_ONLY)
    public boolean isValidIntervalRange() {
        if (initialInterval == null || maxInterval == null) return true;
        return maxInterval > initialInterval;
    }
}